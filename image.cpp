    ...
    const std::string& logic)
{
    ...
    if (!file) {
        std::cerr << "Errore: impossibile aprire " << dotFile << " per scrittura.\n";
        return;
    }
    ...
    for (const auto& t : rts.arches) {
        ...
        std::string label = (t.transition == "tau") ? "τ" : t.transition;
...
void openRTSGraphSVG(const std::string& dotFile, const std::string& svgFile) {
    ...
    if (ret != 0) {
        std::cerr << "Errore nella generazione dell'SVG: comando fallito\n";
        return;
    }
